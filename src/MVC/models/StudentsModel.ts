...
exports.postSingleUser = (student: Student) => {
    if(!student.first_name || !student.last_name || !student.email || !student.student_id || !student.account_type) return Promise.reject({ status: 400, msg: "Bad request" })
...
exports.patchStudent = (student_id: number, student: Student) => {
    if(!student.first_name || !student.last_name || !student.email || !student_id || !student.account_type) return Promise.reject({ status: 400, msg: "Bad request" })
...
exports.fetchStudentAssignements = (student_id: number) => {
    ...
    WHERE students_year.student_id = $1 AND students_subjects.student_id = $1;`,
        [student_id]
      )
